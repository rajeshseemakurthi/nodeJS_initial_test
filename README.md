...
It will be start to build a Node.js Rest API with Express, Sequelize & MySQL. Here we’ll use Sequelize for interacting with the **MySQL** instance.
...
![enter image description here](https://github.com/rajeshseemakurthi/nodeJS_initial_test.git)
...
```package.json
{
  ...
  "version": "1.0.0",
  "description": "Node.js Rest Api with Express, Sequelize, MySQL(Initializing).",
  "main": "server.js",
  "scripts": {
  ...
  "repository": {
    "type": "git",
    "url": "https://github.com/rajeshseemakurthi/nodeJS_initial_test.git"
  },
  "keywords": [
    "node",
    "rest-api",
  ...
  "homepage": "https://github.com/rajeshseemakurthi/nodeJS_initial_test.git",
  "dependencies": {
    "body-parser": "^1.19.0",
    "cors": "^2.8.5",
    "express": "^4.17.1",
...
server.get("/", (\_req, res) => {
   res.json({ message: "Welcome to node.js rest api application.Note This Porject for Testing purposes.Not for Deployement Ready" });
});
...
create a simple route
server.get("/", (\_req, res) => {
   res.json({ message: "Welcome to node.js rest Application." });
});
